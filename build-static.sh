...
compile_with_fallback() {
    ...
    print_info "尝试编译 $build_type 版本..."
    
    # 方法1: 简化静态链接（推荐）
    if compile_simple_static "$target_name"; then
        return 0
    fi
    
    # 方法2: 完整静态链接
    if compile_full_static "$target_name"; then
        return 0
    fi
    
    # 方法3: 动态链接（作为后备）
    if compile_dynamic "$target_name"; then
        print_warning "静态链接失败，使用动态链接作为后备"
        return 0
    fi
...
compile_simple_static() {
    ...
    g++ -std=c++11 -Wall -Wextra -O2 -DNDEBUG -static \
        ...
        src/utils/utils.cpp \
        -lcurl -lssl -lcrypto -lz -ldl -lpthread -lresolv -lnsl -lrt 2>/dev/null
    
    if [[ $? -eq 0 ]]; then
        print_success "简化静态链接编译成功！"
        return 0
    fi
    return 1
}
...
compile_full_static() {
    ...
    g++ -std=c++11 -Wall -Wextra -O2 -DNDEBUG -static \
        ...
        -lpsl -lidn2 -lssh2 -lzstd -lbrotlidec -lbrotlicommon \
        -lnghttp2 -lrtmp -lopenldap -lber -lsasl2 \
        -lgssapi -lkrb5 -lk5crypto -lcom_err -lresolv -lnsl -lrt 2>/dev/null
    
    if [[ $? -eq 0 ]]; then
        print_success "完整静态链接编译成功！"
        return 0
    fi
    return 1
}
...
compile() {
    ...
    case $build_type in
        "dynamic")
            ...
            ;;
        "static-simple")
            compile_simple_static "$target_name"
            ;;
        "static-full")
            compile_full_static "$target_name"
            ;;
        "auto")
            compile_with_fallback "$target_name" "$build_type"
            ;;
        *)
...
# 显示帮助信息
show_help() {
    echo "AIAgent 静态链接编译脚本"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
    echo "  dynamic          - 编译动态链接版本（推荐用于开发）"
    echo "  static-simple    - 编译简化静态链接版本（推荐用于发布）"
    echo "  static-full      - 编译完整静态链接版本（包含所有依赖）"
    echo "  auto             - 自动尝试最佳编译方式（推荐）"
...
main() {
    ...
    case "${1:-help}" in
        "dynamic")
            ...
            ;;
        "static-simple")
            compile "static-simple" "main"
            ;;
        "static-full")
            compile "static-full" "main"
            ;;
        "auto")
            compile "auto" "main"
            ;;
