...
int main() {
    ...
    // 4. 使用agent2_diagnose模块进行诊断分析
    std::cout << "\n【步骤3】正在分析执行计划……" << std::endl;
    DiagnosticReport diagnostic_report = analyze_plan(input);
    std::cout << "诊断完成：" << diagnostic_report.summary << std::endl;

    // 5. 使用agent5_interactive模块判断是否需要用户交互
    bool need_interaction = need_user_interaction(diagnostic_report);
    EnrichedDiagnosticReport enriched_report;
    
    if (need_interaction) {
        std::cout << "\n【步骤4】需要补充信息，正在生成问题……" << std::endl;
        std::string question = generate_question(diagnostic_report);
        std::cout << "问题：" << question << std::endl;
        
        std::string user_answer = multiline_input("请输入你的补充信息（多行，END/#END/两次空行结束，或输入exit/quit退出）：", true);
        if (user_answer == "__USER_EXIT__") {
            std::cout << "\n【用户已选择退出，感谢使用SQL优化助手！】\n" << std::endl;
            return 0;
        }
        
        enriched_report = enrich_report(diagnostic_report, user_answer);
    } else {
        // 如果不需要交互，直接使用基础诊断报告
        enriched_report.base_report = diagnostic_report;
        enriched_report.needs_more_info = false;
    }

    // 6. 使用agent3_strategy模块生成优化策略
    std::cout << "\n【步骤5】正在生成优化策略……" << std::endl;
    OptimizationStrategy strategy = generate_strategy(enriched_report);

    // 7. 使用agent4_report模块输出报告
    std::cout << "\n【步骤6】正在生成优化报告……" << std::endl;
    output_report(strategy);

    // 8. 加载AI模型配置（可选，用于增强分析）
    std::cout << "\n【步骤7】正在加载AI模型配置……" << std::endl;
    ...
    if (load_ai_config("config/ai_models.json", models) && !models.empty()) {
        ...
        std::cout << "\n【AI模型配置未找到，跳过AI增强分析】" << std::endl;
        std::cout << "如需使用AI功能，请复制config/ai_models.example.json为config/ai_models.json并配置API密钥。" << std::endl;
    }
